...
app.use((0, cors_1.default)({
    origin: (globalThis.Bun?.env?.NODE_ENV || globalThis.process?.env?.NODE_ENV) === 'production'
...
function createMcpServer() {
    ...
    server.registerTool("star-wars-info", {
        ...
        inputSchema: {
            resource: zod_1.z.enum(["films", "people", "planets", "species", "starships", "vehicles"]).describe("Type of Star Wars resource to query"),
            id: zod_1.z.number().optional().describe("Specific ID to get individual resource (omit to get all)"),
            search: zod_1.z.string().optional().describe("Search term to filter results by name")
        }
        ...
        try {
            ...
            if (data.results) {
                ...
                data.results.forEach((item, index) => {
                    ...
                    if (item.episode_id)
                        formattedData += `   Episode: ${item.episode_id}\n`;
                    if (item.birth_year)
                        formattedData += `   Birth Year: ${item.birth_year}\n`;
                    if (item.climate)
                        formattedData += `   Climate: ${item.climate}\n`;
                    if (item.model)
                        formattedData += `   Model: ${item.model}\n`;
                    if (item.classification)
                        formattedData += `   Classification: ${item.classification}\n`;
                    formattedData += '\n';
                });
            }
            else {
                ...
                if (data.episode_id)
                    formattedData += `Episode: ${data.episode_id}\n`;
                if (data.director)
                    formattedData += `Director: ${data.director}\n`;
                if (data.release_date)
                    formattedData += `Release Date: ${data.release_date}\n`;
                if (data.birth_year)
                    formattedData += `Birth Year: ${data.birth_year}\n`;
                if (data.height)
                    formattedData += `Height: ${data.height}cm\n`;
                if (data.mass)
                    formattedData += `Mass: ${data.mass}kg\n`;
                if (data.hair_color)
                    formattedData += `Hair Color: ${data.hair_color}\n`;
                if (data.eye_color)
                    formattedData += `Eye Color: ${data.eye_color}\n`;
                if (data.gender)
                    formattedData += `Gender: ${data.gender}\n`;
                if (data.diameter)
                    formattedData += `Diameter: ${data.diameter}km\n`;
                if (data.climate)
                    formattedData += `Climate: ${data.climate}\n`;
                if (data.terrain)
                    formattedData += `Terrain: ${data.terrain}\n`;
                if (data.population)
                    formattedData += `Population: ${data.population}\n`;
                if (data.model)
                    formattedData += `Model: ${data.model}\n`;
                if (data.manufacturer)
                    formattedData += `Manufacturer: ${data.manufacturer}\n`;
                if (data.cost_in_credits)
                    formattedData += `Cost: ${data.cost_in_credits} credits\n`;
                if (data.length)
                    formattedData += `Length: ${data.length}m\n`;
                if (data.crew)
                    formattedData += `Crew: ${data.crew}\n`;
                if (data.passengers)
                    formattedData += `Passengers: ${data.passengers}\n`;
                if (data.classification)
                    formattedData += `Classification: ${data.classification}\n`;
                if (data.designation)
                    formattedData += `Designation: ${data.designation}\n`;
                if (data.average_height)
                    formattedData += `Average Height: ${data.average_height}cm\n`;
                if (data.language)
                    formattedData += `Language: ${data.language}\n`;
    ...
    try {
        if (sessionId && transports[sessionId]) {
            ...
            transport = new streamableHttp_js_1.StreamableHTTPServerTransport({
                ...
                enableDnsRebindingProtection: (globalThis.Bun?.env?.NODE_ENV || globalThis.process?.env?.NODE_ENV) === 'production',
                allowedHosts: (globalThis.Bun?.env?.NODE_ENV || globalThis.process?.env?.NODE_ENV) === 'production'
...
// Start server
const PORT = (globalThis.Bun?.env?.PORT || globalThis.process?.env?.PORT) || 3000;
app.listen(PORT, () => {
    console.log(`🌐 Streamable HTTP MCP Server running on port ${PORT}`);
    console.log(`📊 Health check: http://localhost:${PORT}/health`);
    console.log(`🔗 MCP endpoint: http://localhost:${PORT}/mcp`);
    console.log(`🛡️ DNS protection: ${(globalThis.Bun?.env?.NODE_ENV || globalThis.process?.env?.NODE_ENV) === 'production' ? 'enabled' : 'disabled'}`);
});
...
if (globalThis.process) {
    globalThis.process.on('SIGTERM', gracefulShutdown);
}
